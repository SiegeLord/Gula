...
pub const UNSELECTED: Color = Color::from_rgb_f(0.9, 0.7, 0.7);
pub const LABEL: Color = Color::from_rgb_f(0.7 * 0.9, 0.7 * 0.7, 0.7 * 0.7);
...
pub const CONTROL_WIDTH: f32 = 150.;
...
impl MainMenu
{
	pub fn new(state: &game_state::GameState) -> Result<Self>
	{
		...
		widgets.extend([
			vec![Widget::Button(Button::new(w, h, "New Game", Action::Start))],
...
impl InGameMenu
{
	pub fn new(state: &game_state::GameState) -> Self
	{
		...
		let widgets = WidgetList::new(&[
			...
			&[Widget::Button(Button::new(w, h, "Restart", Action::Start))],
			&[Widget::Button(Button::new(
				w,
				h,
				"Quit to Menu",
